...
const Schema = z.object({ otp: z.string().min(4) });
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ cardId: string }> }) {
  try {
    ...
    const data = await tyga.cards.getCredentials(s.tid, cardId, body as unknown as Record<string, unknown>);
    ...
    console.error("[cards/credentials]", err);
